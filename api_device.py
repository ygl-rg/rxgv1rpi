...
async def Sample(deviceids, sample_count):
    """
    :param deviceids:
    :param sample_count:
    :return: {sensorid-> [recs],...}
    """
    try:
        await lock_obj.acquire()
        data_tbl = collections.defaultdict(collections.OrderedDict)
        deviceids = deviceids[0:5]
        sql_str = rg_lib.Sqlite.GenInSql("""select id, nid, name, moduleid,device_no 
                                            from rxg_zb_device where device_no<>? and nid > ? and id in """,
                                         deviceids)
        sql_args = [xy_lib.DeviceNo.XY_SWITCH, 0] + deviceids
        devs = await api_core.BizDB.Query([sql_str, sql_args])
        temp = 100 if sample_count > 100 else sample_count
        for _ in range(temp):
            curr_ts = rg_lib.DateTime.ts()
            for dev in devs:
                await SyncVal(dev)
                if models.ZbDevice.ValsNotEmpty(dev):
                    for idx, val in enumerate(dev['vals']):
                        sensorid = "{0}_{1}".format(dev['id'], idx)
                        data_tbl[sensorid][curr_ts] = {
                                "sensorid": sensorid,
                                "sensor_name": "{0}_{1}".format(dev['name'], idx),
                                "deviceid": dev['id'],
                                "val": val,
                                "cts": curr_ts
                            }
        ...
        for k in data_tbl:
            for k1 in data_tbl[k]:
                res[k].append(data_tbl[k][k1])
        return res
    finally:
        lock_obj.release()
...
async def SampleSensor(deviceids, sample_count):
    """
    :param deviceids: list
    :param sample_count: integer
    :return: {"sensorids":[], "log_tbl": {sensorid->[sample data,...]}}
    """
    sample_tbl = await Sample(deviceids, sample_count)
    result = {
            'sensorids': list(sample_tbl.keys()),
            'log_tbl': sample_tbl
        }
    result['sensorids'].sort()
    return result
